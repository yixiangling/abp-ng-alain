    ...
    handleNonAbpErrorResponse(response: Response) {
        ...
        switch (response.status) {
            case 401:
                ...
                //     this._tokenService.loginUrl
                // );
                self.handleTargetUrl(this._tokenService.loginUrl);
                break;
            case 403:
...
export class AbpHttp extends Http {
    ...
    get(url: string, options?: RequestOptionsArgs): Observable<Response> {
        ...
        return super
            .get(url, options)
            .pipe(map(response => this.configuration.handleResponse(response)), catchError(error => this.configuration.handleError(error)));
    }
